...
(function (global) {
    ...
    const MENU = {};
    let _styleInj = false;
    let _state = { mode: null, difficulty: 'normal', tank: 'assault' };
    ...
    MENU.renderTankWorkshop = function (selectedMode) {
        ...
        TANKS.forEach((tk) => {
            const card = h('div', 'ct-tank-card' + (_state.tank === tk.id ? ' active' : ''));
            card.style.justifyContent = 'center';   // 卡片撑满后内容垂直居中
            ...
            if (_state.tank === tk.id) applyActive(true);
            card.addEventListener('click', () => {
                _state.tank = tk.id;
                cardGrid.querySelectorAll('.ct-tank-card').forEach(x => {
                    x.classList.remove('active');
                    x.style.borderColor = ''; x.style.boxShadow = ''; x.style.background = '';
                });
        ...
        function drawRadar() {
            ...
            for (let i = 0; i < n; i++) {
                ...
                ctx2.fillStyle = tc;   // canvas 不支持 CSS 变量，改用坦克绑定色
        ...
        descBox.textContent = tankInfo[_state.tank] || tankInfo.assault;
        function updateTankDesc() {
            descBox.textContent = tankInfo[_state.tank] || '';
        ...
        deploy.addEventListener('click', () => {
            ...
            if (hudNs && typeof hudNs.startGame === 'function') {
                try {
                    hudNs.startGame(_state.mode, _state.tank, tankColor(), _state.difficulty);
                } catch (e) {
